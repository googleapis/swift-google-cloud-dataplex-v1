  ...
  Sendable
{
  ...
  public init(from decoder: Decoder) throws {
    ...
    if let value = try container.decodeIfPresent(Swift.UInt32.self, forKey: .retryCount) {
      self.retryCount = value
    }
    if let value = try container.decodeIfPresent(Job.Service.self, forKey: .service) {
      self.service = value
    }
    ...
    if let value = try container.decodeIfPresent(Job.Trigger.self, forKey: .trigger) {
      self.trigger = value
    }
