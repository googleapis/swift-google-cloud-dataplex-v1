...
extension Clients {
  final class CmekServiceTransport: CmekServiceStub {
    ...
    public func createEncryptionConfig(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/encryptionConfigs"
          var query = [
    ...
    public func updateEncryptionConfig(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleCloudGax._RoutingMatcher.pathValue(
              request.encryptionConfig.map({ $0.name }),
              matching: [
                .literal("organizations/"), .singleWildcard, .literal("/locations/"),
                .singleWildcard, .literal("/encryptionConfigs/"), .singleWildcard,
              ],
              fieldName: "encryption_config.name")
          else {
            return nil
          }
